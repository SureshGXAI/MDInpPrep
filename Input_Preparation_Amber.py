...
def ligand_charge(inputfile):
    suppl = SDMolSupplier(f'{inputfile}.sdf', removeHs=False)
    for mol in suppl:
        if mol is not None:
...
def main(pdb_name):
    ...
    for res in res_name_list:
        ...
        charge = ligand_charge(f'{pdb_name}_{res}_ligand')

        cmd3 = f'antechamber -i {pdb_name}_{res}_ligand.sdf -fi sdf -nc {charge} -o {pdb_name}_ligand.mol2 -fo mol2 -c bcc -s 1 -pf yes -at gaff -m 1 -rn {res}'
